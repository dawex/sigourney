...
class ServiceOfferingVerifiableCredentialTest {
	...
	static void init() {
		...
		formatProvider.setFormat(Format.SERVICE_OFFERING_VERIFIABLE_CREDENTIAL, "./legalPerson/%s/dataOfferings/%s/vc");
	...
	private static void assertThatClaimsAreValid(String serializedVc) {
		...
		assertThatJsonStringValue("$['id']", serializedVc)
				.isEqualTo("./legalPerson/62b570acb33e417edcb345ee/dataOfferings/62bab5ae84fd784b1541e8f3/vc");
		...
		assertThatJsonListValue("$['credentialSubject']['gx:requiredMeasures']", serializedVc).hasSize(1)
				...
				.satisfies(measure -> {
					...
					assertThat(measure).containsEntry("httpsschema:description", "Required measure description");
	...
	private static ServiceOfferingVerifiableCredential getServiceOfferingVerifiableCredential() {
		return ServiceOfferingVerifiableCredential.builder()
				.compositeId(new ServiceOfferingVerifiableCredential.Id("62bab5ae84fd784b1541e8f3", "62b570acb33e417edcb345ee"))
				...
				.credentialSubject(ServiceOfferingCredentialSubject.builder()
						...
						.providedBy(new ProvidedBy("62b570acb33e417-provider"))
						.termsAndConditions(List.of(TermsAndConditions.builder()
								.url("60f5ab")
