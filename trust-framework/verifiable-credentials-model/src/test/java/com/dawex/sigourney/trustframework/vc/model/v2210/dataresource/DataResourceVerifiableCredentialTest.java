...
class DataResourceVerifiableCredentialTest {
	...
	static void init() {
		...
		formatProvider.setFormat(Format.DATA_RESOURCE_COPYRIGHT_OWNED_BY,
				"./api/secure/participant/organisations/%s/verifiableCredential");
	...
	private static void assertThatSerializedVcIsValid(String serializedVc) {
		...
		TestUtils.assertThatJsonListValue("$['credentialSubject']['gx:copyrightOwnedBy']", serializedVc)
				.hasSize(1)
				.first()
				.isEqualTo("./api/secure/participant/organisations/62b573deb33e417edcb34-copyrightOwnedBy/verifiableCredential");
